# cross tissue but files from egene
...
for (i in tissues) {
  ...
  tmp <- suppressWarnings(fread(paste0(rdsf_personal,gtex_path,i),
                                stringsAsFactors = FALSE,
                                data.table = FALSE))
...
tmp_input_exp <- tmp_input_exp[,c("slope","ref","alt","gene_name","tissue","rs_id_dbSNP151_GRCh38p7",
...
input_exp$tissue <- gsub(".v8.egenes.txt.gz","",input_exp$tissue)
